...
function update_ani_home_right_time(t1, t2){
    ...
    if (!data.animation.during_start && (!data.animation.change || (t1 === -1 && t2 === -1))){
...
function update_math(elements = null, retry = 5){
    try {MathJax.typeset(elements);}
    catch {
        if (!retry) return;
        let command = function (){
            set_content();
            update_math(elements, retry - 1);
        };
...
function set_page(){
    ...
    if (get_section() === "3" && get_id() !== -1 && get_article() === null){
...
async function set_article_page(){
    ...
    try {file = to_dom(ccc.np(await read_file(ccc.np(content.location))));} catch {file = null;}
    if (file === null) {
...
async function load_data(){
    ...
    data.assets.blogs = to_dom(ccc.np(await read_file(ccc.np("ASSETS/POSTS/blogs.html"))));
    data.assets.articles = JSON.parse(await read_file(ccc.np("ASSETS/POSTS/articles.json")));
    data.assets.categories = JSON.parse(await read_file(ccc.np("ASSETS/POSTS/categories.json")));
}
